...
const Quiz = {
  ...
  next() {
    ...
    console.log('[Quiz] next()', { currentIndex: quiz.currentIndex, total, questionsLength: this.questions.length });
    ...
    if (quiz.currentIndex < total - 1) {
      ...
      // 答题完成，显示结果
      console.log('[Quiz] 答题完成，调用 complete()');
      this.complete();
    }
  },
  ...
  complete() {
    try {
      ...
      console.log('[Quiz] 答题完成，准备显示结果', { product, currentCode, answers: state.quiz.answers });
      ...
      console.log('[Quiz] 计算结果:', result);
      
      Result.render(result);
      
      // 如果有体验码，保存结果到 CodeManager（支持24小时内重复登录查看）
      if (currentCode && typeof CodeManager !== 'undefined') {
        console.log('[Quiz] 保存结果到 CodeManager:', currentCode);
        CodeManager.markUsed(currentCode, result);
      }
      
      // 跳转到结果页
      console.log('[Quiz] 跳转到结果页');
      Router.navigate('page-result', { animation: 'fade' });
    } catch (error) {
      console.error('[Quiz] 完成答题时出错:', error);
      // 即使出错也尝试跳转，避免用户卡死
      try {
        Router.navigate('page-result', { animation: 'fade' });
      } catch (e) {
        console.error('[Quiz] 跳转失败:', e);
        UI.showToast('页面跳转失败，请刷新重试', true);
      }
    }
  }
};
