...
const Result = {
   ...
   */
  render(result) {
    console.log('[Result.render] 开始渲染结果:', result);
    
    // 保存结果到 State 供后续使用（先保存，确保数据不丢失）
    State.set({ lastResult: result }, true);

    // 能量图标
    const energyEl = document.getElementById('result-energy');
    if (energyEl) {
      energyEl.innerHTML = result.emoji || '🌸';
    }

    // 结果名称
    const nameEl = document.getElementById('result-name');
    if (nameEl) {
      nameEl.textContent = result.name || '未知类型';
    }
    ...
    if (result.product === 'basic') {
      // 意识之境：显示 AI 明信片（异步生成）
      this.renderAIPostcard(result).catch(err => {
        console.error('[Result.render] AI明信片生成失败:', err);
        // 失败时回退到传统结果
    ...
    console.log('[Result.render] 结果渲染完成');
  },
  ...
  async renderAIPostcard(result) {
    ...
    if (typeof AIImage !== 'undefined') {
      ...
      // AIImage 不可用，回退到传统结果
      console.warn('AIImage not available, falling back to traditional result');
      this.renderTraditionalResult(result);
    }
  },
